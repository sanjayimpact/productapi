  ...
  matchQuery.$and = searchWords.map(word => ({
   ...
   if (sortField == "-1") {
    products = await Product.aggregate([
      ...
      // ➕ Lookup all variants for grand total stock
      {
        $lookup: {
          from: "variants",
          let: { productId: "$_id" },
          pipeline: [
            ...
            {
              $addFields: {
                allStock: { $ifNull: [{ $arrayElemAt: ["$stockData.quantity", 0] }, 0] },
              },
            },
            { $project: { stockData: 0 } },
          ],
      ...
      {
        $addFields: {
          grandTotalStock: { $sum: "$allVariants.allStock" },
        },
      },
      { $project: { defaultVariant: 0, allVariants: 0 } },
    
      // 🟦 Sort by totalInventory or any other dynamic field
      { $sort: { grandTotalStock: sortOrder } },
